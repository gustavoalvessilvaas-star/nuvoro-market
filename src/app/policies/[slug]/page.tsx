...
const policies: Record<string, { title: string; body: string[] }> = {
  ...
  "terms-of-use": { title: "Terms of Use", body: ["By using this site, you agree to use it lawfully, provide accurate checkout information and avoid interfering with the storefront, admin tools or payment flow.", "Product descriptions are written for practical clarity and may be updated as supplier details are verified. Nuvoro Market avoids unsupported health, safety or performance guarantees.", "Prices, availability, discounts, bundles and shipping estimates may change before an order is placed. A checkout confirmation does not guarantee fulfillment if fraud checks, stock limits or supplier issues require cancellation.", "Payments are processed by Stripe. You agree to provide accurate billing and shipping information and to resolve payment questions through support when needed.", "Shipping timelines are estimates. Nuvoro Market is not responsible for carrier delays, customs-style delays, force majeure events or wrong addresses submitted by the customer beyond the support remedies stated in store policies.", "To the maximum extent allowed by law, Nuvoro Market is not liable for indirect, incidental or consequential damages from use of the site or products.", "Account access is intended for the customer who created the account. Admin access is separate and restricted to approved admin users.", "These terms are starter operational language and should be reviewed before public launch."] },
  "refund-policy": { title: "Refund Policy", body: ["Customers should contact support within 30 days of delivery if an item arrives damaged, defective, missing parts, wrong or materially different from the order.", "Refund eligibility may require photos, order information, delivery confirmation and supplier review. Do not discard damaged packaging until support has replied.", "If an item is missing from a multi-item order, support may verify split shipments, supplier fulfillment records and tracking before approving a replacement or refund.", "Approved refunds are returned to the original payment method when possible. Processing time may vary based on Stripe, the card issuer and bank rules, and can take several business days after approval.", "Shipping charges, return shipping and non-returnable products should be confirmed before launch based on final supplier terms.", "Orders affected by clear fulfillment mistakes, duplicate payment or confirmed non-delivery should be reviewed promptly by the admin team."] },
  "shipping-policy": { title: "Shipping Policy", body: ["Nuvoro Market is prepared for United States shipping at launch. International shipping should remain disabled until supplier, tax and return processes are ready.", "Processing time is typically 2-4 business days before shipment, unless a product page states otherwise or supplier review requires additional time.", "Estimated delivery times are shown on product pages and may vary by supplier, destination, warehouse, weather, carrier delays and order review.", "Tracking is provided when available from the supplier or carrier. Customers can use the order tracking page with order ID and email after tracking is added by an admin.", "Some products may ship separately if fulfilled by different suppliers. Delivery estimates are not guarantees unless a final supplier agreement says otherwise.", "Customers are responsible for entering a complete, accurate shipping address at checkout. Address changes after payment may not be possible once fulfillment begins.", "If a package is delayed, support can help review tracking and supplier status. If a package appears lost, support may request carrier confirmation before replacement or refund review."] },
  "return-policy": { title: "Return Policy", body: ["Return instructions must be requested from support before sending items back. Unapproved returns may not be accepted by the supplier or warehouse.", "The return review window is designed around 30 days after delivery for damaged, wrong, defective or materially different items.", "Some products may not be returnable if used, damaged after delivery, missing parts, hygienic in nature or restricted by supplier policy.", "Return costs, restocking rules and warehouse addresses depend on the selected supplier and should be confirmed before launch.", "If a return is approved, customers should package the item securely and provide tracking when available.", "Non-returnable examples may include final-sale items, used hygiene-sensitive products, items damaged by misuse and products returned without approval.", "The admin team should record return status, supplier response and refund decision in order notes for clear customer support history."] },
    ...
    <>
      <section className="dark-section">
        <div className="container-page py-12">
          <p className="text-xs font-black uppercase tracking-[0.18em] text-aqua">Nuvoro Market Policy</p>
          <h1 className="mt-3 text-4xl font-black text-white">{policy.title}</h1>
          <p className="mt-3 max-w-3xl text-sm font-medium leading-6 text-white/70">This template is operational guidance and is not formal legal advice. Obtain legal review before launch.</p>
        </div>
      </section>
      <section className="container-page -mt-8 py-12">
        <article className="grid gap-4">
          {policy.body.map((paragraph, index) => (
            <div key={paragraph} className="card-surface p-5">
              <p className="text-xs font-black text-blue">Section {index + 1}</p>
              <p className="mt-2 leading-7 text-ink/75">{paragraph}</p>
            </div>
          ))}
        </article>
      </section>
