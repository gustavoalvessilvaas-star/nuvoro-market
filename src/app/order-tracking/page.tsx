...
export default function OrderTrackingPage() {
  ...
  return (
    <section className="container-page grid gap-8 py-10 lg:grid-cols-[420px_1fr]">
      <form onSubmit={submit} className="dark-section h-fit rounded-[1.5rem] p-6 shadow-soft">
        <p className="eyebrow">Order status</p>
        <h1 className="mt-2 text-3xl font-black text-white">Track Your Order</h1>
        <p className="mt-2 text-sm text-white/70">Enter your order ID and email to view fulfillment status.</p>
        <label className="mt-5 grid gap-1"><span className="text-sm font-bold text-white">Order ID</span><input className="field" name="order_id" required /></label>
        <label className="mt-4 grid gap-1"><span className="text-sm font-bold text-white">Email</span><input className="field" type="email" name="customer_email" required /></label>
        <button className="btn-primary mt-5 w-full gap-2"><Search className="h-4 w-4" /> {loading ? "Searching..." : "Search"}</button>
        {error ? <p className="mt-4 rounded-md bg-red-50 p-3 text-sm text-red-700">{error}</p> : null}
      </form>
      <div className="card-surface p-5">
        <div className="flex items-center gap-3">
          <span className="grid h-11 w-11 place-items-center rounded-2xl bg-mint text-moss"><PackageCheck className="h-5 w-5" /></span>
