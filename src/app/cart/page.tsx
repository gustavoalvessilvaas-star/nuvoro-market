      ...
      {items.length ? (
        <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_380px]">
          <div className="grid gap-4">
            {items.map((item) => (
              <article key={item.product.id} className="card-surface grid grid-cols-[96px_1fr] gap-4 p-4">
                ...
                <div className="grid gap-3">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <button aria-label="Remove item" onClick={() => removeItem(item.product.id)} className="grid h-9 w-9 place-items-center rounded-full border border-line bg-white hover:border-coral hover:text-coral"><Trash2 className="h-4 w-4" /></button>
                  </div>
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-2">
                      <button className="grid h-9 w-9 place-items-center rounded-full border border-line bg-white" onClick={() => updateQuantity(item.product.id, item.quantity - 1)} aria-label="Decrease quantity"><Minus className="h-4 w-4" /></button>
                      <span className="w-8 text-center font-bold">{item.quantity}</span>
                      <button className="grid h-9 w-9 place-items-center rounded-full border border-line bg-white" onClick={() => updateQuantity(item.product.id, item.quantity + 1)} aria-label="Increase quantity"><Plus className="h-4 w-4" /></button>
                    </div>
